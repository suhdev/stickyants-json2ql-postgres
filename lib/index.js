...
var SqlCondition = /** @class */ (function () {
    ...
    SqlCondition.fromJson = function (json) {
        ...
        c.refiners = c.refiners && c.refiners.map(function (e) { return SqlCondition.fromJson(e); });
        return c;
    };
    SqlCondition.id = 1;
    return SqlCondition;
...
function createQuery(model) {
    ...
    return [
        model.getSqlQuery()
            .replace(/([^a-zA-Z0-9])(@[a-zA-Z][a-zA-Z0-9]+)([^a-zA-Z0-9])/g, function (_, prefix, key, postfix) { return "" + prefix + map[key] + postfix; }),
        params
    ];
}
